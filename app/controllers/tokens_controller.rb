class TokensController < ApplicationController
  ...
  # GET /tokens/1.json
  def show
    cc = Cryptocompare::Price.full(@token.sym, 'USD')
    cc = cc["RAW"][@token.sym]["USD"]
    @current = {}
    @current[:price] = cc["PRICE"]
    @current[:change] = cc["CHANGEPCT24HOUR"]

    @ticks = @token.ticks.where(
  ...
  private
    ...
    def token_params
      params.require(:token).permit(:name, :sym)
    end
end
